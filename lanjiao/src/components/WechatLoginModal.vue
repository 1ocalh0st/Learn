...
async function loadQrcode() {
  ...
  try {
    // 获取微信授权 URL（实际项目中会用到）
    const res = await request<{ code: string; data: { url: string } }>("/api/auth/wechat/url");
    // 这里可以用 url 生成二维码或跳转
    console.log("WeChat auth URL:", res.data.url);
  } catch (e: any) {
    // 如果微信未配置，使用模拟模式
    console.log("WeChat not configured, using mock mode");
  } finally {
...
async function useMockLogin() {
  ...
  try {
    ...
    }>("/api/auth/wechat/mock", {
      method: "POST",
      body: JSON.stringify({
        nickname: `微信用户_${Math.random().toString(36).substring(2, 6)}`,
      }),
    });
...
<style>
/* 全局样式，用于模态框 */
.wechat-login-modal .arco-modal-header {
